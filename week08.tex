...
\begin{enumerate}
    \item A nonzero vector $\vec v ∈ V$ is called an \emph{eigenvector} of $T$ if for some 
        scalar $λ ∈ \R$
        \[ T(\vec v) = λ \vec v  \]
        \ble{That is, $\vec v$ is only stretched by $T$.}
...
\end{enumerate}
\pr
\ble{One also says: `$\vec v$ is an eigenvector of $T$ with eigenvalue $λ$'}
...
\q{\textbf{Question}}
\lb
\q{But for which $λ ∈ \R$ can we expect to find eigenvectors?}
